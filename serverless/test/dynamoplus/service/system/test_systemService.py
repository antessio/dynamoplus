...
class TestSystemService(unittest.TestCase):
    ...
    def test_update_authorization_http_signature(self, mock_repository, mock_update):
        ...
        expected_model = Model(collection_name + "#" + expected_client_id, collection_name, expected_client_id, document)
    ...
    def test_delete_authorization_http_signature(self, mock_repository, mock_delete):
        ...
        mock_repository.assert_called_once_with(system_table_name)
        self.assertEqual(call(collection_name + "#" + expected_client_id, collection_name), mock_delete.call_args_list[0])
    ...
    def test_createIndexDuplicated(self, mock_repository, mock_create, mock_query):
        ...
        expected_index_model = Model("index#"+expected_id, "index", expected_id, target_index)
        ...
        mock_query.assert_called_once_with(index_metadata, Eq("name", expected_id), Index(None, index_metadata.name, ["name"], None),None,1)


    # @patch.object(Repository, "query_v2")
    # @patch.object(Repository, "create")
    # @patch.object(Repository, "__init__")
