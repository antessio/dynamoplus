...
class TestSystemService(unittest.TestCase):
    ...
    def test_create_aggregation(self, mock_repository, mock_create):
        ...
        calls = [call(expected_model),
                 call(Model("aggregation#" + aggregation.name, "aggregation#collection.name", aggregation.collection_name, target_agg))]
        mock_create.assert_has_calls(calls)
    ...
    def test_collection_count_aggregation(self, mock_repository, mock_increment_counter):
        ...
        self.assertEqual(call(AtomicIncrement("collection#example", "collection", [Counter("count", Decimal(1))])), mock_increment_counter.call_args_list[0])
    ...
    def test_collection_count_aggregation_remove(self, mock_repository, mock_increment_counter):
        ...
        self.assertTrue(mock_repository.called)
        self.assertEqual(call(AtomicIncrement("collection#example", "collection", [Counter("count", Decimal(1),False)])),
    ...
    def test_collection_avg_insert(self, mock_repository, mock_increment_counter):
        ...
        aggregation = Aggregation("example", AggregationType.AVG,
                                  [AggregationTrigger.INSERT, AggregationTrigger.DELETE,AggregationTrigger.UPDATE], "rate", None, None)
        document = {
            "id": 1,
            "name": "whatever",
            "rate": 4
