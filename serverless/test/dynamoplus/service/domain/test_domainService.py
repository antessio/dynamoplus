...
from aws.dynamodb.dynamodbdao import DynamoDBDAO, DynamoDBModel, QueryResult
...
class TestDomainService(unittest.TestCase):
    ...
    def test_createDocument(self, mock_repository, mock_create):
        ...
        expected_model = DynamoDBModel("example#" + expected_id, "example", "1", document)
    ...
    def test_find_all_documents(self, mock_query):
        ...
        mock_query.return_value = QueryResult([
            DynamoDBModel("example#1", "example", "1", {"id": "1", "attribute1": "1"}),
            DynamoDBModel("example#2", "example", "2", {"id": "2", "attribute1": "1"})
        ])
        # when
